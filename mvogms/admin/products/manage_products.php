...
<div class="container-fluid">
	<form action="" id="category-form">
		...
		<div class="form-group">
			...
			<option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Duyệt</option>
			<option value="1" <?php echo isset($status) && $status == 1 ? 'selected' : '' ?>>Chờ duyệt</option>
			</select>
		</div>
...
<script>
	...
	$(document).ready(function(){
		$('#uni_modal #category-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=update_status_product",

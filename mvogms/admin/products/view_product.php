...
<div class="container-fluid">
	...
	<dl>
        ...
        <dd class="pl-3"><?= isset($name) ? $code."-".$vendor : "" ?></dd>
        <dt class="text-muted">Sản phẩm</dt>
        <dd class="pl-3"><?= isset($name) ? $id."-".$name : "" ?></dd>
        ...
        <dd class="pl-3">
            ...
            $status = isset($status) ? $status : '';
                switch($status){
                    case 0:
                        echo '<span class="badge badge-secondary bg-gradient-secondary px-3 rounded-pill">Duyệt</span>';
                        break;
                    case 1:
                        echo '<span class="badge badge-primary bg-gradient-primary px-3 rounded-pill">Chờ duyệt</span>';
                        break;
                    default:
                        ...
                        ?>
                <?php if($status != 0): ?>
                <span class="pl-2"><a href="javascript:void(0)" id="update_status_product">Cập nhật trạng thái</a></span>
            <?php endif; ?>
        </dd>
    </dl>
...
<script>
    $(function(){
        $('#update_status_product').click(function(){
            uni_modal_second("Cập nhật trạng thái sản phẩm - <b><?= isset($id) ? $id : '' ?></b>","products/update_status_product.php?id=<?= isset($id) ? $id : '' ?>")
        })
    })

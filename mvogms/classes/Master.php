...
Class Master extends DBConnection {
	...
	function delete_shop_type(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM shop_type_list where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
 ...
 // function save_shop_type(){
	...
	function delete_category(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Xóa sản phẩm thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_status_product(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `product_list` set `status` = '{$status}' where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái sản phẩm đã cập nhật thành công.";
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái sản phẩm đã cập nhật thất bại.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
...
switch ($action) {
	...
	case 'update_status_product':
		echo $Master->update_status_product();
	break;
	default:
		// echo $sysset->index();
		break;
